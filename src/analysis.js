...
function create_analysis(input_state) {
    ...
    output[step_labels] = new label_cells.CellLabellingState(output[step_inputs], output[step_norm]);
...
export async function loadAnalysis(path, loadFun, { finishFun = null } = {}) {
    ...
    {
        state[step_labels] = label_cells.unserialize(handle, state[step_inputs], state[step_norm]);
