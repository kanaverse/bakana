...
export class InputsState {
    ...
    fetchParameters() {
        ...
        if (output.subset !== null) {
            output.subset.values = output.subset.values.slice();
        }
        return output;
    }
     ...
     * This should contain `field`, a string specifying a field of the column annotation; and `values`, an array of allowed values for that annotation.
    ...
    if ("subset" in phandle.children) {
        ...
        if ("cells" in shandle.children) {
            ...
            if ("indices" in schandle.children) {
                ...
                subset = {
                    field: schandle.open("field", { load: true }).values[0],
                    values: schandle.open("values", { load: true }).values
                };
            } else {
                throw new Error("unrecognized specification for 'subset'");
            }
